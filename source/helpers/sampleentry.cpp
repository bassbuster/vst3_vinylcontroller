...
namespace Steinberg {
  ...
  //           // OverlapCursorFirst.Clear();
		...
		unsigned SampleEntry::AnalyseWavHeader(BYTE * Header) {
			if ((Header[0] == 'R') && (Header[1] == 'I') && (Header[2] == 'F')
				&& (Header[3] == 'F')) {
				unsigned iRiffSise = GetContainerSize(Header + 4);
				return iRiffSise;
			}
			else {
				return 0;
		...
		unsigned SampleEntry::AnalyseWavForm(BYTE * Form) {
			if ((Form[0] == 'W') && (Form[1] == 'A') && (Form[2] == 'V') &&
				(Form[3] == 'E') && (Form[4] == 'f') && (Form[5] == 'm') &&
				(Form[6] == 't') && (Form[7] == ' ')) {
				unsigned iFormLength = GetContainerSize(Form + 8);
				return iFormLength;
			}
			else {
				return 0;
		...
		bool SampleEntry::AnalysePCMCodec(BYTE * Form, unsigned & nCannels,
		...
		unsigned SampleEntry::GetContainerSize(BYTE * Container) {
		...
		bool SampleEntry::isDataContainer(BYTE * Container) {
		...
		bool SampleEntry::isAcidContainer(BYTE * Container) {
		...
		bool SampleEntry::isLoop(BYTE * Container) {
		...
		bool SampleEntry::isOneShoot(BYTE * Container) {
		...
		BYTE SampleEntry::GetAcidBeats(BYTE * Container) {
			return Container[20];
		}

		DWORD SampleEntry::GetChannelData(BYTE * Buffer, unsigned channel,
			unsigned BitsPerSample) {
			DWORD CannelData;
			if (Buffer[BitsPerSample / 8 - 1 +
				channel * BitsPerSample / 8] >= 0)
				CannelData = 0.;
			else
				CannelData = 0xffffffff;
			for (int k = BitsPerSample / 8 - 1; k >= 0; k--) {
				CannelData = (CannelData << 8) +
					(Buffer[k + channel * BitsPerSample / 8] & 0xff);
			}
			return CannelData;
		}

		Sample32 SampleEntry::ConvertToSample(DWORD CannelData, BYTE Type,
			unsigned BitsPerSample) {
			/////////Supported bitrates 8/16/24/32(IEEE Float)
			Sample32 FSample;
			if (Type == 1) {
				if (BitsPerSample == 8) {
					FSample = ((int)CannelData) / 127.0;
				}
				if (BitsPerSample == 16) {
					FSample = ((short)CannelData) / 32767.0;
				}
				if (BitsPerSample == 24) {
					FSample = ((int)CannelData) / 8388607.0;
				}
			}
			else if (BitsPerSample == 32) { // 32bit IEEE Float
				float * SamplePointer = (float *)(&CannelData);
				FSample = *SamplePointer;
			}
			else
				FSample = 0.f;
			return FSample;
		...
		bool SampleEntry::AnalyseContainers(BYTE * Buffer, unsigned BufferSize,
			...
			if (iFormLength > 0) {
				...
				if (AnalysePCMCodec(Buffer, nCannels, iSamplesPerSec,
					iBitsPerSample)) {
					while (iCursor < (BufferSize - 8)) {
						...
						if (isDataContainer(Buffer + iCursor)) {
							...
							SoundBufferLength =
								iFormLength / (nCannels * iBitsPerSample / 8);
							SoundBufferLeft =
								new Sample32[SoundBufferLength + 1];
							SoundBufferRight =
								new Sample32[SoundBufferLength + 1];

							BYTE * Data = Buffer + iCursor + 8;
							for (unsigned i = 0; i < iFormLength;
							i = i + nCannels * iBitsPerSample / 8) {

								for (unsigned j = 0; j < nCannels; j++) {
									DWORD CannelData =
										GetChannelData(Data + i, j,
										iBitsPerSample);
									Sample32 FSample =
										ConvertToSample(CannelData, Buffer[12],
										iBitsPerSample);

									if (nCannels < 2) {
										SoundBufferLeft[i /
										(nCannels * iBitsPerSample / 8)] =
										FSample;
										SoundBufferRight[i /
										(nCannels * iBitsPerSample / 8)] =
										FSample;
						...
						if (isAcidContainer(Buffer + iCursor)) {
  ...
  //           BYTE * Header = (BYTE*)LoadResource(/*(HMODULE)*/hInstance, FindResource(/*(HMODULE)*/hInstance, ResourceName, 1/*RT_RCDATA*/));
		...
		Sample32* SampleEntry::GetBuffer(channel channelname) {
			...
			case kBufferLeftChannel:
				return SoundBufferLeft;
			case kBufferRightChannel:
				return SoundBufferRight;
			default:
				return 0;
			}
		}
		...
		{
			if ((long)position < SoundBufferLength) {
				switch (channelname) {
				case kBufferLeftChannel:
		...
		Sample32 SampleEntry::GetPeakSample(unsigned from_position,
			...
			Sample32 peak = 0;
			if ((long)from_position > SoundBufferLength) {
				return 0;
			}
			if ((long)to_position > SoundBufferLength) {
				to_position = SoundBufferLength;
			}
		...
		Sample64 SampleEntry::GetNoteLength(Sample64 _note, Sample64 _Tempo) {
			if ((Sync) && (ACIDbeats > 0)) {
				return SoundBufferLength / ACIDbeats * _note;
			}
			else if (_Tempo > 0) {
		...
		Sample64 SampleEntry::CalcTempoSpeed(Sample64 _Speed, Sample64 _Tempo, Sample64 _SampleRate)
		{
		   ...
		   if ((ACIDbeats > 0) && (_SampleRate > 0)) {
				...
				return dir * SoundBufferLength * _Tempo / 60.0 / _SampleRate / ACIDbeats;
		   }else
		   if (_SampleRate > 0) {
				return dir * SoundBufferLength * _Tempo / 60.0 / _SampleRate / defaultBeats;
		   }
		...
		void SampleEntry::PlayStereoSampleTempo(Sample32 * Left, Sample32 * Right,
			...
			bool _changeCursors) {
				...
				if ((newSpeed/newTempoSpeed)<=1.3) {
					if (CheckOverlapEvent(PushCue,newTempoSpeed)&&(SmoothOverlap <= 0.00005)) {OverlapStrobe(newTempoSpeed,fabs(newSpeed));plus = true;}
					//else if (CheckSyncroEvent(PushCue,newTempoSpeed)) {SyncStrobe();minus = true;}
					else RealCursor = OverlapCursorFirst;
				}else if ((newSpeed/newTempoSpeed)>1.3) {
					if (CheckStratchEvent(PushCue,newSpeed,newTempoSpeed)&&(SmoothOverlap <= 0.00005)) {StratchStrobe(PushCue);/*plusp = true;*/}
				...
				if (SmoothOverlap>0.0) {
				   ...
				   Sample32 CorrectorCoef = (1.0 - cos(SmoothOverlap*2.0*Pi))/10.0;
		...
		void SampleEntry::OverlapStrobe(double _Speed,double _Tune) {
			if (_Speed<0)
				OverlapCursorSecond.SetCue(beatLength * NormalizeStretchBeat((long)(RealCursor.IntegerPart / beatLength)) - beatOverlap*_Tune/_Speed,0);
            else
				OverlapCursorSecond.SetCue((long)beatLength * (long)NormalizeStretchBeat((long)(RealCursor.IntegerPart / beatLength) + 1) - beatOverlap*_Tune/_Speed,0);
				//OverlapCursorSecond.SetCue((SoundBufferLength/ACIDbeats) * NormalizeBeat((long)(RealCursor.IntegerPart / beatLength) + Sign(_Speed)) - beatOverlap*_Speed,0);
			NormalizeCue(OverlapCursorSecond);
			RealCursor = OverlapCursorFirst;
			SmoothOverlap = 1;
			//FaderProcess = true;
		}

		/*void SampleEntry::StratchStrobe(double _Speed) {
			if (_Speed<0)
				OverlapCursorSecond.SetCue(beatLength * NormalizeStretchBeat((long)(OverlapCursorFirst.IntegerPart / beatLength) + 1),0);
			else
				OverlapCursorSecond.SetCue(beatLength * ((long)(OverlapCursorFirst.IntegerPart / beatLength)),0);
			NormalizeCue(OverlapCursorSecond);
			SmoothOverlap = 1;
		}*/

		void SampleEntry::StratchStrobe(CuePoint &_cue) {
			/*OverlapCursorFirst = OverlapCursorSecond;
			if (_Speed<0)
				OverlapCursorSecond.SetCue(beatLength * NormalizeStretchBeat((long)(RealCursor.IntegerPart / beatLength) + 1),0);
			else
				OverlapCursorSecond.SetCue(beatLength * ((long)(RealCursor.IntegerPart / beatLength)),0);
			NormalizeCue(OverlapCursorSecond); */
			OverlapCursorFirst = OverlapCursorSecond;
			OverlapCursorSecond = _cue;
			SmoothOverlap = 1;
			//FaderProcess = true;
		...
		bool SampleEntry::CheckStratchEvent(CuePoint &_cue,Sample64 _speed,Sample64 _speedtempo){
			bool result = false;
			/*CuePoint _cueNew(_cue);
			_cueNew -= _speedtempo;
			NormalizeCue(_cueNew);
			double beatStragth = ((double)beatLength)/(double)(abs((int)(_speed/_speedtempo))+1.0);
			if (((long)(_cueNew.IntegerPart / beatLength))!=((long)(_cue.IntegerPart / beatLength))) {
					result = false;OverlapCursorFirst = _cue;
			}else
			if (((long)(_cueNew.IntegerPart / beatStragth))!=((long)(_cue.IntegerPart / beatStragth))) {
					result = true;
			}           */
			//if (fabs(OverlapCursorSecond.IntegerPart - _cue.IntegerPart)>fabs((double)beatLength*_speed/_speedtempo)) {
			if ((_speedtempo>0)&&(_cue > OverlapCursorSecond)){
				if (fabs(OverlapCursorSecond.IntegerPart+SoundBufferLength - _cue.IntegerPart)>fabs((double)beatLength/2.0)) {
					result = true;
}
			}else
			if ((_speedtempo<0)&&(_cue < OverlapCursorSecond)) {
				if (fabs(OverlapCursorSecond.IntegerPart - _cue.IntegerPart+SoundBufferLength)>fabs((double)beatLength/2.0)) {
					result = true;
				}
			}else
			if (fabs(OverlapCursorSecond.IntegerPart - _cue.IntegerPart)>fabs((double)beatLength/2.0)) {
				result = true;
			}
			return result;
		}
		...
		bool SampleEntry::CheckRestratchEvent(CuePoint &_cue,Sample64 _offset){
			bool result = false;
			CuePoint _cueNew(_cue);
			_cueNew += _offset;
			NormalizeCue(_cueNew);
		...
		void SampleEntry::CalcNewCursor(Sample64 _offset, CuePoint & _newCursor)
		{

			_newCursor = RealCursor;
			...
			if (!Loop) {
				if ((_newCursor.IntegerPart == SoundBufferLength - 1) &&
					(CurrentSpeed > 0)) {
					return;
				}
			...
			NormalizeCue(_newCursor);
			/*Sample64 fFloatCursor = _newCursor.FloatPart + CurrentSpeed;
			long iIntegerCursor = _newCursor.IntegerPart;
			long IntegerPart = (long)fFloatCursor;
			if (IntegerPart != 0) {
				fFloatCursor = fFloatCursor - (Sample32)IntegerPart;
				iIntegerCursor = iIntegerCursor + IntegerPart;
			}
			if (fFloatCursor < 0) {
				fFloatCursor = 1.0 + fFloatCursor;
				iIntegerCursor--;
			}
			if ((Loop) || ((iIntegerCursor >= 0) && (iIntegerCursor <
				SoundBufferLength))) {
				int32 LoopMultiple = abs(iIntegerCursor / SoundBufferLength);

				if ((iIntegerCursor >= SoundBufferLength)) {

					iIntegerCursor =
						iIntegerCursor - LoopMultiple * SoundBufferLength;

				}
				if (iIntegerCursor < 0) {
					iIntegerCursor = (LoopMultiple + 1) * SoundBufferLength +
						iIntegerCursor;
				...
				if (iIntegerCursor >= SoundBufferLength) {
					...
					iIntegerCursor = SoundBufferLength - 1;
					fFloatCursor = 0;

				}
				else if (iIntegerCursor < 0) {
					iIntegerCursor = 0;
					fFloatCursor = 0;

				}
			}
			_newCursor.SetCue(iIntegerCursor, fFloatCursor);
			if ((Loop) || ((_newCursor.IntegerPart >= 0) && (_newCursor.IntegerPart <
				SoundBufferLength))) {
				int32 LoopMultiple = abs(_newCursor.IntegerPart / SoundBufferLength);

				if ((_newCursor.IntegerPart >= SoundBufferLength)) {

					_newCursor.IntegerPart =
						_newCursor.IntegerPart - LoopMultiple * SoundBufferLength;

				}
				if (_newCursor.IntegerPart < 0) {
					_newCursor.IntegerPart = (LoopMultiple + 1) * SoundBufferLength +
						_newCursor.IntegerPart;
				...
				if (_newCursor.IntegerPart >= SoundBufferLength) {
					...
					_newCursor.IntegerPart = SoundBufferLength - 1;
					_newCursor.FloatPart = 0;

				}
				else if (_newCursor.IntegerPart < 0) {
					_newCursor.IntegerPart = 0;
					_newCursor.FloatPart = 0;
		...
		void SampleEntry::NormalizeCue(CuePoint &_cue){
			if ((Loop) || ((_cue.IntegerPart >= 0) && (_cue.IntegerPart <
				SoundBufferLength))) {
				//int32 LoopMultiple = abs(_cue.IntegerPart / SoundBufferLength);

				if ((_cue.IntegerPart >= SoundBufferLength)) {

					_cue.IntegerPart = _cue.IntegerPart % SoundBufferLength;
						//_cue.IntegerPart - LoopMultiple * SoundBufferLength;

				}
				if (_cue.IntegerPart < 0) {
					_cue.IntegerPart = SoundBufferLength + _cue.IntegerPart % SoundBufferLength;
					//(LoopMultiple + 1) * SoundBufferLength + _cue.IntegerPart;
				...
				if (_cue.IntegerPart >= SoundBufferLength) {
					...
					_cue.IntegerPart = SoundBufferLength - 1;
					_cue.FloatPart = 0;

				}
				else if (_cue.IntegerPart < 0) {
					_cue.IntegerPart = 0;
					_cue.FloatPart = 0;
		...
		bool SampleEntry::MoveCursor(Sample64 _offset) {
			if (SoundBufferLength >= 4) {
				CalcNewCursor(_offset, RealCursor);
				return true;
			}
			else
				return false;
		...
		void SampleEntry::PlayStereoSample(Sample32 * Left, Sample32 * Right,
			...
			if (SoundBufferLength >= 4) {
				...
				CuePoint NewCursor;
				CalcNewCursor(_offset, NewCursor);
				...
				if (NewCursor.IntegerPart < SoundBufferLength - 2) {
					Point2 = SoundBufferLeft[NewCursor.IntegerPart + 1];
				}
				if (NewCursor.IntegerPart < SoundBufferLength - 3) {
		...
		void SampleEntry::setCursorOnBeat(unsigned _BeatNumber){
			...
			//beatCueEnd.FloatPart = 0;
			if (ACIDbeats == 0) {
				RealCursor.IntegerPart = 0;
				//beatCueEnd.IntegerPart = SoundBufferLength;
				currentBeat = 0;
				...
				RealCursor.IntegerPart = (SoundBufferLength/ACIDbeats)*(_BeatNumber);
				//beatCueEnd.IntegerPart = (SoundBufferLength/ACIDbeats)*(_BeatNumber+1);
		...
		unsigned SampleEntry::NormalizeStretchBeat(long _Beat){
			if (ACIDbeats>0) {
				if (Loop) {
					return (_Beat >= (ACIDbeats*beatOverlapMultiple))?(_Beat % (ACIDbeats*beatOverlapMultiple)):((_Beat < 0)?((ACIDbeats*beatOverlapMultiple) + (_Beat % (ACIDbeats*beatOverlapMultiple))):((unsigned) _Beat));
				}else{
					return (_Beat >= (ACIDbeats*beatOverlapMultiple))?ACIDbeats*beatOverlapMultiple-1:0;
				}
			}else
			if (defaultBeats>0) {
				if (Loop) {
					return (_Beat >= (defaultBeats*beatOverlapMultiple))?(_Beat % (int)(defaultBeats*beatOverlapMultiple)):((_Beat < 0)?(defaultBeats*beatOverlapMultiple + (_Beat % (int)(defaultBeats*beatOverlapMultiple))):((unsigned) _Beat));
				}else{
					return (_Beat >= (defaultBeats*beatOverlapMultiple))?defaultBeats*beatOverlapMultiple-1:0;
				}
			}else  return 0;
		}
